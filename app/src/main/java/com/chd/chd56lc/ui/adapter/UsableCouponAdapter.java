...
public class UsableCouponAdapter extends CommonAdapter<CouponBean.Item> {
    private String seletetedId;

    public UsableCouponAdapter(Context context, List<CouponBean.Item> datas, String seletetedId) {
        super(context, R.layout.item_my_coupon, datas);
        this.seletetedId = seletetedId;
    }

    @SuppressLint("SetTextI18n")
    @Override
    protected void convert(ViewHolder holder, CouponBean.Item item, int position) {
        if (item.getId().equals(seletetedId)) {
