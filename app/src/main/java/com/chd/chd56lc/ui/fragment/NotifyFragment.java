...
public class NotifyFragment extends BaseFragment implements IMessageListView {
    ...
    protected void initView() {
        ...
        tvNoData.setText("暂无平台信息");
        if (messageBeans == null)
            messageBeans = new ArrayList<>();
        notifyAdapter = new NotifyAdapter(context, messageBeans);
        ...
        notifyAdapter.setOnItemClickListener(new MultiItemTypeAdapter.OnItemClickListener() {
            ...
            public void onItemClick(View view, RecyclerView.ViewHolder holder, int position) {
                toActivity(new Intent(context, NotifyDetailActivity.class)
                        .putExtra(ID, messageBeans.get(position).getId()));
            }
...
//        initDate();
    ...
    private void initDate() {
        messagePresenter.paginageMsgList(page, count, 2);
    }
    ...
    public void updateMessageList(PageListBean<MessageBean> messageBeanPageListBean) {
        if (messageBeanPageListBean != null && messageBeanPageListBean.getArray() != null && messageBeanPageListBean.getArray().size() > 0) {
            ...
            messageBeans.addAll(messageBeanPageListBean.getArray());
